import * as React from "react";
import { IEmojiProps } from "../../styled";

const SvgPlantOther4 = (props: IEmojiProps) => (
  <svg viewBox="0 0 72 72" width="1em" height="1em" {...props}>
    <path
      fill="#5C9E31"
      d="M56.035 54.005l-8.123-15.21 3.346.54a.998.998 0 001.018-1.496l-7.372-12.434 3.077.218a.998.998 0 00.89-1.565L36.825 6.724a1.019 1.019 0 00-.834-.428.998.998 0 00-.822.453l-3.717 5.7-7.821 12.008a.998.998 0 00.998 1.53l2.301-.374-7.552 12.212a.998.998 0 00.998 1.512l3.727-.563-8.135 15.231a.998.998 0 00.947 1.466l12.447-.836 1.036-.07 10.961-.125 13.718 1.031h.075a.998.998 0 00.88-1.468l.003.002z"
    />
    <g fill="none" stroke="#000" strokeLinecap="round" strokeWidth={2}>
      <path
        strokeLinejoin="round"
        d="M41.448 53.49l13.746 1.033-9.09-17.02 5.344.862-8.36-14.1 4.985.35-12.07-17.37L24.449 24.99l4.47-.724-8.72 14.099 5.705-.862-9.09 17.02 13.51-.908"
      />
      <path strokeMiterlimit={10} d="M36.004 55.964v9.309" />
    </g>
  </svg>
);

export default SvgPlantOther4;
