import * as React from "react";

const SvgEmotion33 = (props: React.SVGProps<SVGSVGElement>) => (
  <svg viewBox="0 0 72 72" width="1em" height="1em" {...props}>
    <ellipse cx={36} cy={36} rx={32.5} ry={13.5} fill="#3F3F3F" />
    <path
      fill="#9B9B9A"
      d="M36 32.5c13.52 0 25.107 3.303 30.005 8 1.605-1.54 2.495-3.228 2.495-5 0-7.18-14.55-13-32.5-13s-32.5 5.82-32.5 13c0 1.772.89 3.46 2.495 5 4.898-4.697 16.486-8 30.005-8z"
    />
    <g fill="none" stroke="#000" strokeMiterlimit={10} strokeWidth={2}>
      <ellipse cx={36} cy={36} rx={32.5} ry={13.5} />
      <path d="M36 32.5c13.52 0 25.107 3.303 30.005 8 1.605-1.54 2.495-3.228 2.495-5 0-7.18-14.55-13-32.5-13s-32.5 5.82-32.5 13c0 1.772.89 3.46 2.495 5 4.898-4.697 16.486-8 30.005-8z" />
    </g>
  </svg>
);

export default SvgEmotion33;
