import * as React from "react";

const SvgHandSingleFinger36 = (props: React.SVGProps<SVGSVGElement>) => (
  <svg viewBox="0 0 72 72" width="1em" height="1em" {...props}>
    <g fill="#fcea2b">
      <path d="M20.238 52.066c1.076 8.6 8.044 14.985 16.215 14.857 9.106 0 16.855-6.833 16.855-16.878l-.002.251-.619-11.213c-.168-2.194-1.916-3.87-3.989-3.826h-9.42c-5.634-3.222-18.864 6.587-18.864 8.51" />
      <path d="M23.671 48.469c-1.905-.153-4.002-1.634-3.889-3.7l.054-14.782c-.238-1.933 3.313-3.816 5.097-4.074 1.785-.257 2.473 2.984 2.712 4.917.988-5.098.247-.932-.742 6.639v7c.113 2.066-1.326 3.847-3.232 4z" />
      <path d="M31.534 53.238c-1.906-.153-3.345-1.934-3.232-4l-.029-19.77c-.687-1.807-5.285-1.227 2.908-3.292 1.754-.442 2.994.718 3.232 2.65.038.31.39 5.102.353 5.412v15c.113 2.066-1.326 3.847-3.232 4z" />
      <path d="M33.76 37.933v-7.31c-.022-1.916 2.194-5.08 4.213-5.337 2.02-.258 6.92 1.697 7.19 3.63.043.31.017 1.121-.026 1.431l-2.028 5.844" />
      <path d="M43.109 37.63l.366-29.207c0-6.175 9.417-5.389 9.417.283l1.055 30.954" />
    </g>
    <g fill="none" stroke="#000" strokeLinecap="round" strokeWidth={2}>
      <path
        strokeLinejoin="round"
        d="M24.312 40.898a3.786 3.786 0 01-3.5-4v-7a3.531 3.531 0 117 0v7a3.786 3.786 0 01-3.5 4z"
      />
      <path
        strokeMiterlimit={10}
        d="M53.31 51.072V40.729c-.164-2.228-1.784-4.537-4.814-4.537M48.496 36.198h-8.29c-1.711-.12-3.188 1.413-3.315 3.442.151 2.076 1.546 3.716 3.296 3.878l.87.115c1.812.238 3.295 1.32 3.295 2.401v2.4c-1.785.45-3.437 1.454-4.788 2.91"
      />
      <path
        strokeMiterlimit={10}
        d="M20.78 53.06c1.059 8.458 7.913 14.737 15.95 14.612 8.957 0 16.58-6.72 16.58-16.6l-.002-.001M44.308 8.372v14.989M52.683 8.58v24.819M44.308 8.372c0-5.875 8.375-5.188 8.375.208"
      />
      <path
        strokeLinejoin="round"
        d="M32.749 40.76c-3.322 1.228-4.997-1.934-4.875-4v-7a3.531 3.531 0 117 0"
      />
      <path
        strokeLinejoin="round"
        d="M34.937 34.126v-5.318a3.531 3.531 0 117 0l.03 3.504"
      />
      <path strokeMiterlimit={10} d="M20.78 53.06c-.08-1.512 0-8.94 0-8.94" />
    </g>
  </svg>
);

export default SvgHandSingleFinger36;
